...
def polyfit(df: pd.DataFrame, headers: list[str], poly_degree: int, size: str):
  ...
  coeff_str = " + ".join([f"{c:.3g}x^{i}" if i > 0 else f"{c:.3g}" 
  ...
  with PdfPages(pdf_buffer) as pdf:
      ...
      if std_err is not None:
          std_err_str = "\n".join([f"Std. Error (x^{i}): {se:.4g}" 
                                    for i, se in enumerate(reversed(std_err))])
          stats_text += f"\nStandard Errors:\n{std_err_str}\n"
      stats_text += f"\n\nCovariance Matrix:\n{cov_str}"
...
def expfit(df: pd.DataFrame, headers: list[str], size: str):
    x = df[headers[0]].to_numpy(dtype=float)
    y = df[headers[1]].to_numpy(dtype=float)
...
def logfit(df: pd.DataFrame, headers: list[str], size: str):
    x = df[headers[0]].to_numpy(dtype=float)
    y = df[headers[1]].to_numpy(dtype=float)
...
def gaussfit(df: pd.DataFrame, headers: list[str], size: str):
    x = df[headers[0]].to_numpy(dtype=float)
    y = df[headers[1]].to_numpy(dtype=float)
...
def powfit(df: pd.DataFrame, headers: list[str], size: str):
    x = df[headers[0]].to_numpy(dtype=float)
    y = df[headers[1]].to_numpy(dtype=float)
...
def poisson_model(x, lambd):
    return (lambd**x * np.exp(-lambd)) / np.factorial(x)

def poissonfit(df: pd.DataFrame, headers: list[str], size: str):
    x = df[headers[0]].to_numpy(dtype=int)  # Ensure integer values for discrete counts
    y = df[headers[1]].to_numpy(dtype=float)
    params, covariance = curve_fit(poisson_model, x, y, p0=[np.mean(x)])  # Initial guess for λ
    fig_size = (7, 3) if size == "large" else (3.375, 3)
    fig, ax = plt.subplots(figsize=fig_size)
    ax.scatter(x, y, label="Data", color="blue", alpha=0.5)
